...
collectedLE<- collectedL[which(collectedL$site == 'BART' | collectedL$site == 'HARV' | collectedL$site == 'BLAN' | collectedL$site == 'SCBI' | collectedL$site == 'SERC' | collectedL$site == 'OSBS' | collectedL$site == 'STEI' | collectedL$site == 'TREE' | collectedL$site == 'UNDE' | collectedL$site == 'GRSM' | collectedL$site == 'MLBS' | collectedL$site == 'ORNL' | collectedL$site == 'TALL'),]


#'HARV' |
                                      # collectedL$site == 'SCBI' |
                               #collectedL$site == 'ORNL'),]
# collectedL$site = factor(collectedL$site, 
#                             levels = c('HARV',
#                                        'SCBI',
#                                        'ORNL'),
#                             labels = c('HARV - NORTHEAST',
...
ggplot(collectedLE, aes(y = sum_countsL, x = Weeks, group = site, color = site))+
  #geom_smooth(se = FALSE)+
  geom_area(position = 'stack', aes(fill = site), alpha = .8)+
#aes(fill = site),alpha = .8)+
  clean_background +
  geom_vline(xintercept = 12.5, alpha = .3, ) +
  geom_vline(xintercept = 23.5, alpha = .3, ) +
  geom_vline(xintercept = 48.5, alpha = .3, ) +
  geom_vline(xintercept = 73.5, alpha = .3, ) +
  geom_vline(xintercept = 102.5, alpha = .3, ) +
  geom_vline(xintercept = 127.5, alpha = .3, ) +
  labs(y = 'Summed larval counts\n (2016 - 2019)', x = 'Collection Dates') +
  #labs(fill = "Site")+
  theme(axis.title.y =  element_text(face = 'bold',size = 12))+
  theme(axis.title.x =  element_text(face = 'bold',size = 12))+
  #scale_x_continuous(breaks = round(seq(min(collected$Month), max(collected$Month))))+
  theme(axis.title.y = element_text(margin = margin(t = 0, r = 15, b = 0, l = 0))) +
  theme(legend.position = c(.08, .52))+
  theme(legend.position = 'bottom', legend.box = 'horizontal')+
  theme(axis.text.x = element_text(angle = 90, size = 6, vjust = 0.5))+
  annotate(geom = "text", x = 17.5, y = 2600, label = "May", size = 3.5) + 
  annotate(geom = "text", x = 29, y = 2600, label = "June", size = 3.5) +
  annotate(geom = "text", x = 53, y = 2600, label = "July", size = 3.5)+
  annotate(geom = "text", x = 82, y = 2600, label = "August", size = 3.5)+
  annotate(geom = "text", x = 114, y = 2600, label = "September", size = 3.5)+
  annotate(geom = "text", x = 137, y = 2600, label = "October", size = 3.5)+
  coord_cartesian(ylim = c(0,2550))
ggsave("geomarealarvaweeks.pdf", dpi = 320)
