...
optimal$site = factor(optimal$site, 
 ...
 # lets smooth this out 
  ...
  #ggsave("geom_keysites_weeks_v1.pdf", dpi = 320)
...
ggplot(Larvalburden, aes( x= Month, y = larvaeYN))+
  ...
  theme(legend.position = 'bottom', legend.box = 'horizontal')
ggsave("propmammalburdens.pdf", dpi = 320)
...
ggplot(firstnymph, mapping = aes(x = moonth, y = site))+
  ...
  geom_segment(aes(x=3,xend=4,y=8,yend=8), size = .2, alpha = .3)+
   geom_segment(aes(x=1,xend=2,y=6,yend=6), size = .2, alpha = .3) +  
  geom_segment(aes(x=1,xend=2,y=3,yend=3), size = .2, alpha = .3)+
  geom_segment(aes(x=2,xend=3,y=2,yend=2), size = .2, alpha = .3)+
  theme(axis.text.x = element_text(angle = 45, size = 8, vjust = 0.5))+
  ytitletheme + xtitletheme +
  clean_background +
...
northeast <- larvalburden[which(larvalburden$site == 'BART' | larvalburden$site == 'HARV'),]
  ...
  geom_jitter(color = '#4682B4', alpha = .8)+
  facet_wrap(~site)+
  theme(strip.text = element_text(size=5))+
  clean_background+
  labs(y = 'Proportion of mammals with tick burdens', x = 'Collection Months')
