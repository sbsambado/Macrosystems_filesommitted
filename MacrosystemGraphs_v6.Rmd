...
**1. Time scale of sampling.  Are there multiple tick sampling events at a site within a year?**
...
**2. Host density: Are there estimates of host density from particular sites and are there multiple estimates of host density within a year?**
...
ggplot(yeartrapsite, aes(x = Year, y = freq))+
  ...
  ytitletheme + xtitletheme
ggsave("MammalSamplingFrequencyYear_v1.pdf", dpi = 320)
...
ggplot(monthtrapsite, aes(x = Month, y = freq))+
  ...
  ytitletheme + xtitletheme
ggsave("MammalSamplingFrequencyMonth_v1.pdf", dpi = 320)

```

**3. What are the data like (if they exist) for B. burgdorferi prevalence at particular sites?**

Incidence across selected sites (>150 ticks tested)
```{r}
tick_path <- read.csv("/Users/samanthasambado/Macrosystems/filesToStack10092/stackedFiles/tck_pathogen.csv", header=T)
...
# remove NA                                     
#tick_path_Bb_col_E   <- subset(tick_path_Bb_col_E, !is.na(plotID))                           
x <- ggplot(tick_path_Bb_col_E, aes(y = incidence, x = plotID))+
  geom_boxplot(fill = 'gray91', na.rm = TRUE)+
  scale_x_discrete(limits = (levels(tick_path_Bb_col_E$plotID)))+
  clean_background +
  theme(axis.text.y = element_text(size = 5.7, vjust = 0.9))+
  titleytheme +
  geom_hline(yintercept = c(6.4, 12.4, 18.4, 25.4),color = 'red', size = .4, alpha = .3)+
  labs(y = "B. burgdorferi incidence", x = "plotID", title = 'B. burgdorferi incidence in questing ticks') +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 1.04)) +
  xtitletheme +
  rotatexaxistheme
ggsave("BBincidenceSiteBoxplot_v1.pdf", dpi = 320)
...
ggsave("BbIncidenceTicksPlotID_v1.pdf", dpi = 320)
  ...
  # annotate(geom = "text", x = .9, y = 37.1, label = "hHEAST", size = 3.5, color = 'red')+
...
tick_path_Bb_col   <- subset(tick_path_Bb_col, !is.na(plotID))   


ggplot(tick_path_Bb_col, aes(x = incidence, y = plotID))+
  geom_boxplot(fill = 'gray91', na.rm = TRUE)+
  ...
  titleytheme +
  #geom_hline(yintercept = c(6.4, 12.4, 18.4, 25.4),color = 'red', size = .4, alpha = .3)+
  labs(x = "B. burgdorferi incidence", y = "plotID", title = 'B. burgdorferi incidence in questing ticks') +
  scale_x_continuous(expand = c(0, 0), limits = c(0, 1.04)) +
  xtitletheme 


pos<- tick_path_Bb_col %>% 
  mutate(possites = incidence) %>% 
  group_by(plotID) %>% 
  filter(incidence > 0.0) %>%
  #filter(Date == min(collectDate)) %>% 
  #select(larvaeYN == 1)%>%
  slice(1) %>% # takes the first occurrence if there is a tie
  ungroup()

ggplot(pos, aes(x = incidence, y = plotID))+
  geom_bar(stat = 'identity',fill = 'gray79', na.rm = TRUE)+
  #scale_y_discrete(limits = rev(levels(tick_path_Bb_col_E$plotID)))+
  clean_background +
  theme(axis.text.y = element_text(size = 5.7, vjust = 0.9))+
  titleytheme +
...
ggplot(possite, aes(x = incidence, y = site))+
  geom_bar(stat = 'identity',fill = 'gray79', na.rm = TRUE)+
  #scale_y_discrete(limits = rev(levels(tick_path_Bb_col_E$plotID)))+
  clean_background +
  theme(axis.text.y = element_text(size = 5.7, vjust = 0.9))+
  titleytheme +
  #geom_hline(yintercept = c(6.4, 12.4, 18.4, 25.4),color = 'red', size = .4, alpha = .3)+
  labs(x = "B. burgdorferi incidence", y = "site", title = 'B. burgdorferi incidence in questing ticks') +
  #scale_x_continuous(expand = c(0, 0), limits = c(0, 1.04)) +
  xtitletheme+
  ylim( "OSBS","TALL", "ORNL","UKFS", "KONZ",
        "SCBI", "BLAN","SERC","TREE", "HARV" )
ggsave("BbIncidenceTicksSite_v1.pdf", dpi = 320)
...
# filter out sites
sitesynchrony <- Iscap[which(Iscap$site != 'DELA' & Iscap$site != 'KONA' & Iscap$site != 'KONZ' & Iscap$site != 'LENO' & Iscap$site != 'UKFS' & Iscap$site != 'JORN' & Iscap$site != 'GRSM' & Iscap$site != 'ABBY' & Iscap$site != 'UNDE' & Iscap$site != 'BART' & Iscap$site != 'OAES' & Iscap$site != 'MLBS'),]


sitesync <- sitesynchrony   %>%
...
#ggsave("geomaregionweeks.pdf", dpi = 320)
  ...
  northeast <- Iscap[which(Iscap$site == 'BART' | Iscap$site == 'HARV'),]
  
  NE <- northeast   %>%
  group_by(Weeks, site, sexOrAge) %>%
  summarise(sum_counts = sum(individualCount), .groups = 'drop')
  
  ggplot(NE, aes(y = log(sum_counts), x = Weeks, group = sexOrAge, color = sexOrAge, fill =sexOrAge ))+
  geom_density(stat = 'identity', alpha = .6)+
  facet_wrap(~ site, scale = 'free_y', ncol = 3)+
  clean_background  +
  theme(axis.text.x = element_text(angle = 90, size = 4, vjust = 0.5))+
...
## Midwest
  midwest <- Iscap[which(Iscap$site == 'UNDE' | Iscap$site == 'TREE'| Iscap$site == 'STEI' ),]
  
  MW <-  midwest   %>%
  group_by(Weeks, site, sexOrAge) %>%
  summarise(sum_counts = sum(individualCount), .groups = 'drop')
  
  ggplot(MW, aes(y = log(sum_counts), x = Weeks, group = sexOrAge, color = sexOrAge, fill =sexOrAge ))+
  geom_density(stat = 'identity', alpha = .6)+
  facet_wrap(~ site, scale = 'free_y', ncol = 3)+
  clean_background  +
  theme(axis.text.x = element_text(angle = 90, size = 4, vjust = 0.5))+
  ...
  midatlantic <- Iscap[which(Iscap$site == 'SERC' | Iscap$site == 'BLAN'| Iscap$site == 'SCBI' ),]
  
  MA <-   midatlantic  %>%
  group_by(Weeks, site, sexOrAge) %>%
  summarise(sum_counts = sum(individualCount), .groups = 'drop')
  
  ggplot(MA, aes(y = log(sum_counts), x = Weeks, group = sexOrAge, color = sexOrAge, fill =sexOrAge ))+
  geom_density(stat = 'identity', alpha = .6)+
  facet_wrap(~ site, scale = 'free_y', ncol = 1)+
  clean_background  +
  theme(axis.text.x = element_text(angle = 90, size = 4, vjust = 0.5))+
  theme(legend.position = 'bottom', legend.box = 'horizontal')+
  labs(y = 'Log(tick counts) (2016 - 2019)', x = 'Collection Dates')+
...
appalachians <- Iscap[which(Iscap$site == 'MLBS' | Iscap$site == 'ORNL'| Iscap$site == 'GRSM' ),]
  
  AP <-   appalachians  %>%
  group_by(Weeks, site, sexOrAge) %>%
  summarise(sum_counts = sum(individualCount), .groups = 'drop')
  
  ggplot(AP, aes(y = log(sum_counts), x = Weeks, group = sexOrAge, color = sexOrAge, fill =sexOrAge ))+
  geom_density(stat = 'identity', alpha = .6)+
  facet_wrap(~ site, scale = 'free_y', ncol = 3)+
  clean_background  +
  theme(axis.text.x = element_text(angle = 90, size = 4, vjust = 0.5))+
 ...
 ## ozarks and southeast
    
ozse <- Iscap[which(Iscap$site == 'TALL' | Iscap$site == 'OSBS'),]
  
  OZSE <-  ozse  %>%
  group_by(Weeks, site, sexOrAge) %>%
  summarise(sum_counts = sum(individualCount), .groups = 'drop')
  
  ggplot(OZSE, aes(y = log(sum_counts), x = Weeks, group = sexOrAge, color = sexOrAge, fill =sexOrAge ))+
  geom_density(stat = 'identity', alpha = .6)+
  facet_wrap(~ site, scale = 'free_y', ncol = 3)+
  clean_background  +
  theme(axis.text.x = element_text(angle = 90, size = 4, vjust = 0.5))+
...
optimal <- Iscap[which(Iscap$site == 'HARV' | Iscap$site == 'SCBI' | Iscap$site == 'ORNL' | Iscap$site == 'OSBS'),]

# ^ took out BLAN & SERC TREE, but it could be included, think TREE should be included
#unique(optimal$site)
optimal$site = factor(optimal$site, 
                             levels = c("HARV","SCBI",  "ORNL", "OSBS"),
                    labels = c("HARV - Northeast", "SCBI - Mid Atlantic",  "ORNL - Appalachians", "OSBS - Southeast"))
  ...
  ggplot(Optimal, aes(y = log(sum_counts), x = Weeks, group = sexOrAge,fill = sexOrAge))+
  geom_density(stat = 'identity', alpha = 1)+ #alpha = .9
  facet_wrap(~ site, scale = 'free_y', ncol = 1)+
  clean_background  +
  theme(axis.text.x = element_text(angle = 90, size = 4, vjust = 0.5))+
  theme(legend.position = 'bottom', legend.box = 'horizontal')+
  ...
  theme(axis.title.y =  element_text(face = 'bold',size = 12, vjust = .5))+
  theme(axis.title.x =  element_text(face = 'bold',size = 12))+
  scale_fill_manual(values = c('#4682B4', '#B4464B'))+
  labs(fill = "Life stage")+
   guides(fill = guide_legend(order = 1),color = FALSE)+
    theme(legend.title = element_text(size = 7),legend.text = element_text(size = 7))+
    #scale_x_discrete(breaks = Key$Weeks[c(T,F,F, F, F, F, F, F,F, F, F, F, F,  F, F, F, F, F, F, F)]) +
  ...
  library(ggplot2)
 # lets smooth this out 
  ggplot(Optimal, aes(y = log(sum_counts), x = Weeks, group = sexOrAge,fill = sexOrAge))+
  #geom_density(stat = 'identity')+ #alpha = .9
    stat_smooth(geom = 'area', method = 'loess', span = .2, alpha = 1)+
  facet_wrap(~ site, scale = 'free_y', ncol = 1)+
  clean_background  +
  theme(axis.text.x = element_text(angle = 90, size = 4, vjust = 0.5))+
  #theme(legend.position = 'bottom', legend.box = 'horizontal')+
  labs(y = 'Log(tick counts) for 2016 - 2019', x = 'Collection Dates')+
  theme(strip.background = element_rect(fill = 'gray95'))+
  theme(strip.text.x = element_text(face = 'bold')) +
  ...
  labs(fill = "Life stage")+
   #guides(fill = guide_legend(order = 1),color = FALSE)+ 
    theme(legend.title = element_text(size = 7),legend.text = element_text(size = 7))+
    #scale_x_discrete(breaks = Key$Weeks[c(T,F,F, F, F, F, F, F,F, F, F, F, F,  F, F, F, F, F, F, F)]) +
    theme(legend.position = c(.005, .965), legend.text = element_text(face = "bold"),
          legend.title = element_text(face = "bold"))+
    theme(legend.box.margin = margin(0, 0, 0, 0)) +
    theme(legend.key.size = unit(.5, "cm"), legend.key.width = unit(0.3,"cm"), legend.justification = "left") +
    #theme(legend.direction = "horizontal")+
    ylim(0,6)+
    geom_vline(xintercept = 12.5, alpha = .3 ) +
    geom_vline(xintercept = 30.5, alpha = .3 ) +
    geom_vline(xintercept = 51.5, alpha = .3 ) +
    geom_vline(xintercept = 73.5, alpha = .3 ) +
    geom_vline(xintercept = 100.5, alpha = .3 ) +
    geom_vline(xintercept = 120.5, alpha = .3 )+
  annotate(geom = "text", x = 21, y = 5.7, label = "May", size = 2.5)+
  annotate(geom = "text", x = 41, y = 5.7, label = "June", size = 2.5) +
  annotate(geom = "text", x = 62, y = 5.7, label = "July", size = 2.5) +
  annotate(geom = "text", x = 87, y = 5.7, label = "August", size = 2.5) +
  annotate(geom = "text", x = 110.7, y = 5.7, label = "September", size = 2.5)
ggsave("TickLifestagePhenologySite_v2.pdf", dpi = 320)
...
Optimal$julian <- as.Date(Optimal$Weeks, format = "%m- %d")
Optimal$julian <- format(Optimal$julian, "%j")
Optimal$week <- round(as.numeric(Optimal$julian)/6)



  ggplot(Optimal, aes(y = log(sum_counts + 1), x = week, group = sexOrAge,fill = sexOrAge))+
    stat_smooth(geom = 'area', method = 'loess', span = .2, alpha = .9)+
  facet_wrap(~ site, scale = 'free_y', ncol = 1)+
  theme_minimal()+
  theme(axis.text.x = element_text(angle = 0, size = 10, vjust = 0.5))+
  labs(y = 'Log(tick counts) for 2016 - 2019', x = 'Weeks')+
  theme(strip.background = element_rect(fill = 'gray95'))+
  theme(strip.text.x = element_text(face = 'bold')) +
  theme(axis.title.y =  element_text(face = 'bold',size = 11, vjust = .5))+
  theme(axis.title.x =  element_text(face = 'bold',size = 11))+
  ...
  ggsave("TickLifestagePhenologySite_v3.pdf", dpi = 320)
...
ggplot(Optimal, aes(x = week, y = log(sum_counts + 1))) + 
  geom_line()+
  facet_grid(site ~ sexOrAge)

ggplot(Optimal, aes(x = week, y = log(sum_counts + 1), color = sexOrAge)) + 
  geom_line()+
  facet_wrap(~site, scale = 'free_y', ncol = 1)+
  theme_minimal()
```
...
First time mammal data point pops up
...
# aggregate larval burden by plot and collect date
mamsTrap_captures_L_col <- aggregate(larvaeYN ~ plotID + collectDate, data=mamsTrap_captures, FUN=mean)
#nrow(mamsTrap_captures_L_col)
#head(mamsTrap_captures_L_col)
...
mamsTrap_captures_L_col$site <- substr(mamsTrap_captures_L_col$plotID, 1, 4)
#head(mamsTrap_captures_L_col)
#unique(mamsTrap_captures_L_col$larvaeYN)
...
# aggregate nymphal burden by plot and collect date
mamsTrap_captures_N_col <- aggregate(nymphsYN ~ plotID + collectDate, data=mamsTrap_captures, FUN=mean)
#nrow(mamsTrap_captures_N_col)
#head(mamsTrap_captures_N_col)
...
## seems like mamsTrap_captures_N_col_E and mamsTrap_captures_L_col_E are the relevant sites, but I need to give them new names cause these are ridiculously long

larvalburden <- mamsTrap_captures_L_col_E

larvalburden$collectDate <- as.factor(larvalburden$collectDate)
larvalburden$plotID <- unlist(larvalburden$plotID)
larvalburden$collectDate <- as.character(larvalburden$collectDate)
larvalburden$Year <- substr(larvalburden$collectDate, 1, nchar(larvalburden$collectDate)-6)
#head(Iscap)

larvalburden$Month <- substr(larvalburden$collectDate, 6, nchar(larvalburden$collectDate)-3)
larvalburden$Weeks <- substr(larvalburden$collectDate, 6, nchar(larvalburden$collectDate))


## nymphs
nymphalburden <- mamsTrap_captures_N_col_E

nymphalburden$collectDate <- as.factor(nymphalburden$collectDate)
nymphalburden$plotID <- unlist(nymphalburden$plotID)
nymphalburden$collectDate <- as.character(nymphalburden$collectDate)
nymphalburden$Year <- substr(nymphalburden$collectDate, 1, nchar(nymphalburden$collectDate)-6)


nymphalburden$Month <- substr(nymphalburden$collectDate, 6, nchar(nymphalburden$collectDate)-3)
nymphalburden$Weeks <- substr(nymphalburden$collectDate, 6, nchar(nymphalburden$collectDate))

# set factors
larvalburden$site = factor(larvalburden$site, 
                             levels = c("BART" ,"HARV", 
                                        "UNDE" ,"TREE","STEI" ,
                                        "BLAN" , "SCBI" ,"SERC",
                                        "MLBS", "ORNL", "GRSM",
...
nymphalburden$site = factor(nymphalburden$site, 
                             levels = c("BART" ,"HARV", 
                                        "UNDE" ,"TREE","STEI" ,
                                        "BLAN" , "SCBI" ,"SERC",
                                        "MLBS", "ORNL", "GRSM",
...
Graphs of mammal burdens
```{r}
ggplot(larvalburden, aes( x= collectDate, y = larvaeYN))+
  geom_point()+
  facet_wrap(~site)+
  theme(strip.text = element_text(size=5))

ggplot(larvalburden, aes( x= Month, y = larvaeYN))+
  geom_jitter()+
  facet_wrap(~site)+
  theme(strip.text = element_text(size=5))
...
ggplot(nymphalburden, aes( x= collectDate, y = nymphsYN))+
  geom_point()+
  facet_wrap(~site)+
  theme(strip.text = element_text(size=5))

ggplot(nymphalburden, aes( x= Month, y = nymphsYN))+
  geom_point()+
  facet_wrap(~site)+
  theme(strip.text = element_text(size=5))
...
# omit OSBS
Larvalburden <- larvalburden[which(larvalburden$site != 'OSBS'),]
Nymphalburden <- nymphalburden[which(nymphalburden$site != 'OSBS'),]

cols <- c("Larva"="#4682B4","Nymph"="#B4464B")
ggplot(Larvalburden, aes( x= Month, y = larvaeYN))+
  geom_jitter(data = Larvalburden, aes(x =  Month, y = larvaeYN, color = 'Larva'),alpha = .5)+
  geom_jitter(data = Nymphalburden, aes(x = Month, y = nymphsYN, color = 'Nymph'), alpha = .5)+
  facet_wrap(~site, scale = 'free_y')+
  theme(strip.text = element_text(size=5, face = 'bold'),strip.background = element_rect(fill = 'gray95'))+
  clean_background+
  labs(y = 'Proportion of mammals with tick burdens \n (2016 - 2019)', x = 'Collection Months')+
  theme(axis.text.x = element_text(angle = 45, size = 8, vjust = 0.5))+
  ytitletheme + xtitletheme +
  scale_color_manual(name="Life stage",values=cols) +
  theme(legend.position = 'bottom', legend.box = 'horizontal')
ggsave("MammalBurdenProportionsSite_v1.pdf", dpi = 320)

  



#ggarrange(lm + rremove('ylab') + rremove('xlab'), 
 #         nm + rremove('ylab') + rremove('xlab'), ncol = 1)


```
When did burdens first pop up
```{r}

as.integer(larvalburden$larvaeYN)

firstylarva<- larvalburden %>% 
  mutate(moonth = Month) %>% 
  group_by(site) %>% 
  filter(larvaeYN == 1) %>%
  filter(moonth == min(Month)) %>%
  #filter(Date == min(collectDate)) %>% 
  #select(larvaeYN == 1)%>%
  slice(1) %>% # takes the first occurrence if there is a tie
  ungroup()

firstylarva$site = factor(firstylarva$site, 
                             levels = c("BART" ,"HARV", 
                                        "UNDE" ,"TREE","STEI" ,
                                        "BLAN" , "SCBI" ,"SERC",
                                        "MLBS", "ORNL", "GRSM",
...
firstnymph$site = factor(firstnymph$site, 
                             levels = c("BART" ,"HARV", 
                                        "UNDE" ,"TREE","STEI" ,
                                        "BLAN" , "SCBI" ,"SERC",
                                        "MLBS", "ORNL", "GRSM",
...
unique(firstnymph$site)
as.factor(firstnymph$site)
as.factor(firstylarva$site)

ggplot(firstnymph, mapping = aes(y = moonth, x = site))+
  geom_point(firstnymph, mapping = aes(y = moonth, x = site, color = 'Nymph'), size = 3)+
  geom_point(firstylarva, mapping = aes(y = moonth, x = site, color = 'Larva'), size = 3)+
  geom_point(aes(x=2, y=2), colour="#4682B4", size = 3)+
  geom_point(aes(x=4, y=1), colour="#4682B4", size = 3) +
  geom_point(aes(x=7, y=3), colour="#4682B4", size = 3) +
  geom_point(aes(x=8, y=2), colour="#B4464B", size = 3) +
  geom_segment(aes(x=2,xend=2,y=2,yend=3), size = .3, alpha = .5)+
  geom_segment(aes(x=4,xend=4,y=1,yend=2), size = .3, alpha = .5)+
  geom_segment(aes(x=5,xend=5,y=1,yend=2), size = .3, alpha = .5) +  
geom_segment(aes(x=8,xend=8,y=1,yend=2), size = .3, alpha = .5)+
  theme(axis.text.x = element_text(angle = 45, size = 8, vjust = 0.5))+
  ytitletheme + xtitletheme +
  clean_background +
  labs(y = 'Collect Month', x = 'Site')


## now do it by collection date
# 
# Firstylarva<- larvalburden %>% 
#   mutate(Date = as.Date(collectDate)) %>% 
#   group_by(site) %>% 
...
cols <- c("Larva"="#4682B4","Nymph"="#B4464B","Overlap" = "#46008B")

ggplot(firstnymph, mapping = aes(x = moonth, y = site))+
  geom_point(firstnymph, mapping = aes(x = moonth, y = site, color = 'Nymph'), size = 3)+
  geom_point(firstylarva, mapping = aes(x = moonth, y = site, color = 'Larva'), size = 3)+
   scale_color_manual(name=c("Life stage", "Overlap"),values=cols) +
   geom_point(aes(x=3, y=12), colour="#B4464B", size = 3)+
   geom_point(aes(x=2, y=10), colour="#B4464B", size = 3) +
   geom_point(aes(x=4, y=8), colour="#B4464B", size = 3) +
   geom_point(aes(x=2, y=4), colour="#B4464B", size = 3) +
   geom_point(aes(x=1, y=3), colour="#B4464B", size = 3) +
   geom_point(aes(x=2, y=2), colour="#B4464B", size = 3) +
   geom_point(aes(x=2, y=12), colour="#4682B4", size = 3) +
   geom_point(aes(x=2, y=10), colour="#4682B4", size = 3) +
   geom_point(aes(x=2, y=3), colour="#4682B4", size = 3) +
   geom_segment(aes(x=2,xend=3,y=12,yend=12), size = .2, alpha = .3)+
  geom_segment(aes(x=3,xend=4,y=8,yend=8), size = .2, alpha = .5)+
   geom_segment(aes(x=1,xend=2,y=6,yend=6), size = .2, alpha = .5) +  
  geom_segment(aes(x=1,xend=2,y=3,yend=3), size = .2, alpha = .5)+
  geom_segment(aes(x=2,xend=3,y=2,yend=2), size = .2, alpha = .5)+
  theme(axis.text.x = element_text(angle = 45, size = 8, vjust = 0.5))+
  ytitletheme + xtitletheme +
  labs(x = 'Collection Month', y = 'Site', title = 'First occurrence of tick burden on mammal')+
  ylim("TALL", "GRSM","ORNL", "MLBS",
       "SERC","SCBI" , "BLAN" , "STEI",
       "TREE", "UNDE" , "HARV",  "BART")+
 annotate(geom = "point", x = 3, y = 11, colour = "#46008B", size = 3.5) +
 annotate(geom = "text", x = 3, y = 10.5, label = "overlap", hjust = "center")+
 annotate(geom = "point", x = 2, y = 10, colour = "#46008B", size = 3.5) +
 #annotate(geom = "text", x = 2, y = 9.5, label = "overlap", hjust = "center")+
 annotate(geom = "point", x = 4, y = 9, colour = "#46008B", size = 3.5) +
 #annotate(geom = "text", x = 4, y = 9.5, label = "overlap", hjust = "center")+
 annotate(geom = "point", x = 1, y = 7, colour = "#46008B", size = 3.5) +
 #annotate(geom = "text", x = 1, y = 6.5, label = "overlap", hjust = "center")+
 annotate(geom = "point", x = 3, y = 5, colour = "#46008B", size = 3.5) +
 #annotate(geom = "text", x = 3, y = 4.5, label = "overlap", hjust = "center")+
 annotate(geom = "point", x = 2, y = 4, colour = "#46008B", size = 3.5) +
 #annotate(geom = "text", x = 2, y = 4.5, label = "overlap", hjust = "center")+
 annotate(geom = "point", x = 1, y = 1, colour = "#46008B", size = 3.5) +
 #annotate(geom = "text", x = 1, y = 1.5, label = "overlap", hjust = "center")+
theme(plot.background = element_rect("white"),
        panel.background = element_rect("white"),
        panel.grid = element_line("gray80"), #altered
        axis.line = element_line("gray25"),
        axis.text = element_text(size = 12, color = "gray25"),
        axis.title = element_text(color = "gray25"),
        legend.text = element_text(size = 12),
        legend.key = element_rect("white"))
ggsave("MammalBurdenFirstOccurenceSite_v1.pdf", dpi = 320)


  
```

Regional mammal burdens
...
northeast <- larvalburden[which(larvalburden$site == 'BART' | larvalburden$site == 'HARV'),]
  
  NE <- northeast   %>%
  group_by(Month, site) %>%
    summarise(prop = mean(larvaeYN), .groups = 'drop')
  
  ggplot(NE, aes( x= Month, y = prop))+
  geom_bar(stat = 'identity',fill = '#4682B4', alpha = .8)+
  facet_wrap(~site)+
  theme(strip.text = element_text(size=5))+
  clean_background+
  labs(y = 'Proportion of mammals with tick burdens', x = 'Collection Months', title = 'Northeast')


   
  midwest <- larvalburden[which(larvalburden$site == 'UNDE' | larvalburden$site == 'TREE'| larvalburden$site == 'STEI' ),]
  ...
  ggplot(MW, aes( x= Month, y = prop))+
  geom_bar(stat = 'identity',fill = '#4682B4', alpha = .8)+
  facet_wrap(~site)+
  theme(strip.text = element_text(size=5))+
  clean_background+
  labs(y = 'Proportion of mammals with tick burdens', x = 'Collection Months', title = 'Midwest')

  
  
  midatlantic <- larvalburden[which(larvalburden$site == 'SERC' | larvalburden$site == 'BLAN'| larvalburden$site == 'SCBI' ),]
  
  MA <-   midatlantic  %>%
  group_by(Month, site) %>%
    summarise(prop = mean(larvaeYN), .groups = 'drop')
  
  ggplot(MA, aes( x= Month, y = prop))+
  geom_bar(stat = 'identity',fill = '#4682B4', alpha = .8)+
  facet_wrap(~site)+
  theme(strip.text = element_text(size=5))+
  clean_background+
  labs(y = 'Proportion of mammals with tick burdens', x = 'Collection Months', title = 'Mid Atlantic')
  
  
  
appalachians <- larvalburden[which(larvalburden$site == 'MLBS' | larvalburden$site == 'ORNL'| larvalburden$site == 'GRSM' ),]
  ...
  ggplot(AP, aes( x= Month, y = prop))+
  geom_bar(stat = 'identity',fill = '#4682B4', alpha = .8)+
  facet_wrap(~site)+
  theme(strip.text = element_text(size=5))+
  clean_background+
  labs(y = 'Proportion of mammals with tick burdens', x = 'Collection Months', title = 'Appalachians')

     
ozse <- larvalburden[which(larvalburden$site == 'TALL' | larvalburden$site == 'OSBS'),]
  
  OZSE <-  ozse  %>%
  group_by(Month, site) %>%
    summarise(prop = mean(larvaeYN), .groups = 'drop')
  
  ggplot(OZSE, aes( x= Month, y = prop))+
  geom_bar(stat = 'identity',fill = '#4682B4', alpha = .8)+
  facet_wrap(~site)+
  theme(strip.text = element_text(size=5))+
  clean_background+
  labs(y = 'Proportion of mammals with tick burdens', x = 'Collection Months', title = 'Ozarks')
...
ggsave("NymphalTickPhenologySite_v1.pdf", dpi = 320)
...
ggsave("NymphalTickPhenologySelectedSites_v1.pdf", dpi = 320)
...
aes(y = individualCount, x = as.Date(collectDate)))+
  ...
  ggsave("NymphalTickPhenologySite2019_v1.pdf", dpi = 320)
...
N2018
ggsave("NymphalTickPhenologySite2018_v1.pdf", dpi = 320)
...
finalnymphquesting
ggsave("NymphalTickPhenologySite2016to2019_v1.pdf", dpi = 320)
