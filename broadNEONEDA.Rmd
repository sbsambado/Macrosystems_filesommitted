...
ticks_taxa2$julian <- as.Date(ticks_taxa2$collectDate, format = "%Y-%m- %d")
ticks_taxa2$julian <- format(ticks_taxa2$julian, "%j")
ticks_taxa2$week <- round(as.numeric(ticks_taxa2$julian)/7)


## round up week
plot(predictorEffects(glm(individualCount ~ week, data = I_scap_N, family = poisson)))
Iscap_Ncol <-aggregate(individualCount ~ week, data = I_scap_N, FUN = sum)
plot(predictorEffects(glm(individualCount ~ week, data = Iscap_Ncol, family = poisson),lines=list(multiline=TRUE),
     lattice=list(key.args=list(columns=2)),
     confint=list(style="bars")))

ticks_taxa2_col <- aggregate(individualCount ~ sexOrAge + week, data = ticks_taxa2, FUN = sum)

ticks_taxa2_colN <- ticks_taxa2_col[which(ticks_taxa2_col$sexOrAge == 'Nymph'),]
as.factor(ticks_taxa2_colN$julian)

ggplot(ticks_taxa2_colN, aes(x = as.factor(week, y = individualCount))+
  geom_point()+
  geom_area()+
  #facet_wrap(~sexOrAge, scale = 'free_y')+
  theme_minimal() +
  theme(legend.position = 'bottom', legend.box = 'horizontal')+
  geom_vline(xintercept = '23', color = 'red')
  
  #geom_vline(xintercept=ticks_taxa2_colN$week[ticks_taxa2_colN$individualCount == max(ticks_taxa2_colN$individualCount)],color = 'red')


ticks_taxa2_col <- aggregate(individualCount ~ sexOrAge + week, data = ticks_taxa2, FUN = sum)

ticks_taxa2_colA <- ticks_taxa2_col[which(ticks_taxa2_col$sexOrAge == 'Female' |
                                            ticks_taxa2_col$sexOrAge == 'Male'),]

adult <- aggregate(individualCount ~ week, data = ticks_taxa2_colA, FUN = sum)
as.factor(ticks_taxa2_colA$julian)

ggplot(adult, aes(x = factor(week), y = individualCount))+
  geom_point()+
  geom_line()+
  #facet_wrap(~sexOrAge, scale = 'free_y')+
  theme_minimal() +
...
#I_scap_N_no0 <- I_scap_N[which(I_scap_N$individualCount != 0),]
as.factor(ticks_taxa2$julian)
ggplot(ticks_taxa2, aes(x = julian, y = individualCount, color = sexOrAge))+
  geom_point()+
  facet_wrap(~sexOrAge, scale = 'free_y')+
  theme_minimal() +
  rotatexaxistheme +
...
plot(predictorEffects(glm(individualCount ~ julian, data = I_scap_N, family = poisson)))

Iscap_Ncol <-aggregate(individualCount ~ julian, data = I_scap_N, FUN = sum)
plot(predictorEffects(glm(individualCount ~ julian, data = Iscap_Ncol, family = poisson),lines=list(multiline=TRUE),
     lattice=list(key.args=list(columns=2)),
     confint=list(style="bars")))
...
ticks_taxa2_col <- aggregate(individualCount ~ sexOrAge + julian, data = ticks_taxa2, FUN = sum)

ticks_taxa2_colN <- ticks_taxa2_col[which(ticks_taxa2_col$sexOrAge == 'Nymph'),]
as.factor(ticks_taxa2_colN$julian)

ggplot(ticks_taxa2_colN, aes(x = julian, y = individualCount, color = sexOrAge))+
  geom_point()+
  facet_wrap(~sexOrAge, scale = 'free_y')+
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, size = 3, vjust = 0.5))+
  theme(legend.position = 'bottom', legend.box = 'horizontal')+
geom_vline(xintercept=ticks_taxa2_colN$julian[ticks_taxa2_colN$individualCount == max(ticks_taxa2_colN$individualCount)],color = 'red')

  #added max line vertical line!
ggplot(ticks_taxa2_col, aes(x = julian, y = individualCount, color = sexOrAge))+
  geom_point()+
  facet_wrap(~sexOrAge, scale = 'free_y')+
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 90, size = 3, vjust = 0.5))+
  theme(legend.position = 'bottom', legend.box = 'horizontal')+
  geom_vline(xintercept=ticks_taxa2_col$julian[ticks_taxa2_col$individualCount == max(ticks_taxa2_col$individualCount)],color = 'red')




plot(predictorEffects(glm(individualCount ~ julian, data = ticks_taxa2_colN, family = poisson)))

plot(ecdf(ticks_taxa2_col[ticks_taxa2_col$sexOrAge == 'Nymph',]$individualCount), col = 'red')
lines(ecdf(ticks_taxa2_col[ticks_taxa2_col$sexOrAge == "Female",]$individualCount), col = 'purple')
lines(ecdf(ticks_taxa2_col[ticks_taxa2_col$sexOrAge == "Male",]$individualCount), col = 'green')
```
...
I_scap_N_colY <- I_scap_N_colY[which(I_scap_N_colY$site != 'DELA' &
                                     ...
                                     I_scap_N_colY$site != 'OSBS' &
                                       I_scap_N_colY$site != 'MLBS' &
                                       I_scap_N_colY$site != 'TALL'),]
...
Tick_path$Month <- substr(Tick_path$Date, 6, nchar(Tick_path$Date)-3)
#head(Tick_path_Bb)
#nrow(Tick_path_Bb)
#View(Tick_path)
...
nrow(Path)
Path <- path %>%
  filter(testResult == 'Positive' | testResult == 'Negative') %>%
  group_by(testPathogenName, Month) %>%
  summarize(tested = length(testResult),.groups = 'drop')


 Path$testPathogenName = factor(Path$testPathogenName,
...
Path <- Path[which(Path$testPathogenName != '<NA>'),]
ggplot(Path, aes(x = Month, y = tested))+
  geom_point(aes(color = testPathogenName))+
  facet_wrap( ~ testPathogenName, scale = 'free_y')+
  rotatexaxistheme+
      theme_bw()+
...
# just nymphs, just published data
published_diffN <- published_diff[which(published_diff$lifestage == 'nymph'),]
#published_diffN <- published_diff[which(published_diff$region != 'midatlantic'),]

ggplot(published_diffN, aes(x = region, y = diff))+
  geom_boxplot(fill = 'grey90')+
        geom_jitter(width = .1,alpha = 0.5, aes(size = tot_nymph, color = region))+
        scale_size(range = c(3, 10),name = 'Total ticks tested',
             breaks = c(100,500,1000))+
  theme_bw() + xtitletheme + ytitletheme +
  labs(x = "Region", y = "Difference in prevalence per site (%)", title = "Difference between Bb & Bm nymphal prevalence (%)")+
  scale_x_discrete(limits = c("westcoast", "midwest", "northeast"),
    labels = c("West", "Midwest", "Northeast"))+
  theme(legend.position = 'bottom', legend.box = 'horizontal')+
guides(color = FALSE)
ggsave("PathogenDifferenceRegionPublished_v4.pdf", dpi = 320)


ggplot(published_diffN, aes(x = region, y = diff))+
  geom_boxplot(fill = 'grey90')+
  #geom_point(size = 3, alpha = .5, aes(color = region))+
  geom_jitter(data = neon_diff, aes(x = region, y = 100*(diff)), size = 3, shape = 23)+
        geom_jitter(width = .1,alpha = 0.5, aes(size = tot_nymph, color = region))+
        scale_size(range = c(3, 10),name = 'Total ticks tested',
             breaks = c(100,500,1000))+
  theme_bw() + xtitletheme + ytitletheme +
...
ggplot(Tick_path_col_no0_path, aes(y = incidence, x = as.numeric(month), color = region))+
   ...
   xtitletheme + ytitletheme+
  ggtitle(incidence ~ plotID + Date + testPathogenName)+
 ggsave("PathogenIncidencebyTypeRegion_v3.pdf", dpi = 320)
 

# Pathcolor <- Path %>%
#   group_by(testPathogenName) %>%
#   summarise(totaltested = sum(tested), .groups = 'drop')

Path$totaltested <- ifelse(Path$testPathogenName == 'Borrelia miyamotoi', "1,282",
                                ifelse(Path$testPathogenName == 'Babesia microti', "1,282",
                                ifelse(Path$testPathogenName == 'Borrelia burgdorferi sl', "1,282",
                                ifelse(Path$testPathogenName == 'Borrelia mayonii', "1,282",
                                ifelse(Path$testPathogenName == 'Anaplasma ph.', "8,559",
                                 ifelse(Path$testPathogenName == 'Ehrlichia chaffeensis', "7,277",
                                        NA))))))
 ...
 Path$testPathogenName = factor(Path$testPathogenName,
                                                   levels = c('Borrelia miyamotoi', 'Babesia microti',
                                                              'Ehrlichia chaffeensis', 
                                                              'Borrelia burgdorferi sl', 'Anaplasma ph.',
                                                              'Borrelia mayonii'
                                                              ))
 
 
ggplot(Path, aes(x = Month, y = tested, fill = totaltested))+
  geom_histogram(stat = 'identity')+
  facet_wrap(~testPathogenName, scale = 'free_y')+
  theme_bw()+
  theme(strip.background = element_rect(fill = 'gray95'))+
  labs(x = 'Month', y = 'Tested', fill = 'Total tested')+
  xtitletheme + ytitletheme +
  theme(legend.position = 'bottom', legend.box = 'horizontal')
 # scale_fill_manual(values = c('#87A0B4FF', '#4883B4FF', '#0062B4FF'))
 ggsave("PathogenIncidencebyTypeTotaltested_v3.pdf", dpi = 320)
...
not aggregated
```{r}
Tick_path_ag <- aggregate(incidence ~ siteID + Month + testPathogenName, data=Tick_path, FUN=mean)

Tick_path_ag <- Tick_path_ag[which(Tick_path_ag$incidence > 0),]
Tick_path_ag$testPathogenName = factor(Tick_path_ag$testPathogenName,
                                                   levels = c('Borrelia miyamotoi', 'Babesia microti',
                                                              'Ehrlichia chaffeensis', 
                                                              'Borrelia burgdorferi sl', 'Borrelia mayonii',
                                                              'Anaplasma ph.'))
...
Tick_path_ag$region <- ifelse(Tick_path_ag$siteID == "HARV", "Northeast",
                                        ifelse(Tick_path_ag$siteID == "BLAN", "Midatlantic",
                                         ifelse(Tick_path_ag$siteID == "SCBI", "Midatlantic",
                                         ifelse(Tick_path_ag$siteID == "SERC", "Midatlantic",
                                         ifelse(Tick_path_ag$siteID == "TREE", "Great lakes",
                                          ifelse(Tick_path_ag$siteID == "UKFS", "Praire peninsula",
                                        ifelse(Tick_path_ag$siteID == "ORNL","Appalachians",
                                        ifelse(Tick_path_ag$siteID == "KONZ","Praire peninsula",
                                        ifelse(Tick_path_ag$siteID == "TALL","Southeast",
                                               ifelse(Tick_path_ag$siteID == "OSBS","Southeast",
                                        NA))))))))))


ggplot(Tick_path_ag, aes(y = incidence, x = as.numeric(Month), color = region))+
    ...
    facet_wrap(~testPathogenName, scale = 'free_y', ncol = 3) +
      ...
      theme(legend.position = 'bottom', legend.box = 'horizontal')+
      labs(x = 'Month', y = 'Incidence', color = 'Region')+
  ggtitle('incidence ~ siteID + Month + testPathogenName')+
   xtitletheme + ytitletheme
ggsave("PathogenIncidencebyTypeRegion_v4.pdf", dpi = 320)



ggplot(Tick_path_ag, aes(y = incidence, x = as.numeric(Month), color = siteID))+
    geom_smooth(color = 'black', aes(ymin = ifelse(..ymin.. < 0, 0, ..ymin..)))+
    geom_point()+
    facet_wrap(~testPathogenName, scale = 'free_y', ncol = 3) +
      theme_bw()+
...
sitepath <- path %>%
  filter(testResult == 'Positive' | testResult == 'Negative') %>%
  group_by(testPathogenName, Month, siteID) %>%
  summarize(tested = length(testResult),.groups = 'drop')

sitepath$testPathogenName = factor(sitepath$testPathogenName,
                                                   levels = c('Borrelia miyamotoi', 'Babesia microti',
                                                              'Borrelia burgdorferi sensu lato', 'Borrelia mayonii',
                                                              'Ehrlichia chaffeensis', 'Borrelia lonestari',
                                                              'Anaplasma phagocytophilum', 'Ehrlichia ewingii'),
...
sitepath$region <- ifelse(sitepath$siteID == "HARV", "Northeast",
                                        ifelse(sitepath$siteID == "BLAN", "Midatlantic",
                                         ifelse(sitepath$siteID == "SCBI", "Midatlantic",
                                         ifelse(sitepath$siteID == "SERC", "Midatlantic",
                                         ifelse(sitepath$siteID == "TREE", "Great lakes",
                                          ifelse(sitepath$siteID == "UKFS", "Praire peninsula",
                                        ifelse(sitepath$siteID == "ORNL","Appalachians",
                                        ifelse(sitepath$siteID == "KONZ","Praire peninsula",
                                        ifelse(sitepath$siteID == "TALL","Southeast",
                                               ifelse(sitepath$siteID == "OSBS","Southeast",
                                        NA))))))))))


sitepath = sitepath[which(sitepath$testPathogenName != 'NA'),]
sitepath = sitepath[which(sitepath$region != 'NA'),]

ggplot(sitepath, aes(x = Month, y = tested, fill = siteID))+
  geom_histogram(stat = 'identity')+
  facet_wrap(~testPathogenName, scale = 'free_y', ncol = 4)+
  theme_bw()+
  theme(strip.background = element_rect(fill = 'gray95'))+
...
ggplot(sitepath, aes(x = Month, y = tested, fill = region))+
  geom_histogram(stat = 'identity')+
  facet_wrap(~testPathogenName, scale = 'free_y', ncol = 4)+
  theme_bw()+
  theme(strip.background = element_rect(fill = 'gray95'))+
  ...
  xtitletheme + ytitletheme +
  theme(legend.position = 'bottom', legend.box = 'horizontal')
 ggsave("PathogenIncidencebyTypeTotaltested_v4.pdf", dpi = 320)
